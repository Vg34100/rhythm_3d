...
namespace StringUtil
{
	...
	std::vector<std::string> split(const std::string & s, char delim, bool skipEmpties)
	{
	...
	std::string replaceAll(std::string str, std::string pattern, std::string replacement, bool startFromBeginning)
	{
	...
	std::string parseIncludes(const std::string & _text, const std::string & dir)
	{
		...
		while (includeFound != std::string::npos)
		{
			...
			if (validInclude)
			{
				...
				{
					std::string includeFilename = text.substr(firstQuote + 1, lastQuote - firstQuote - 1);

					std::string includeRelativePath = dir + "/" + includeFilename;

					std::string includeText = "\n" + IO::readText(includeRelativePath.c_str()) + "\n";

					std::string includeReplace = text.substr(includeFound, lastQuote - includeFound + 1);
