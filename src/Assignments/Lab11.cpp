...
// Variables for Lab 11
namespace cmps_4480_lab_11 {
	...
	std::string skinnedMeshPath = IO::getAssetRoot() + "/" + "cylinder2.gltf";
	...
	void initContext();
	void updateNodeMatrix(s_ptr<GLTFNode> node, bool updateChildren);
	...
	struct IKChain {
		...
		vec3 lastEndEffectorPosition;

		bool renderUI() {
			ImGui::PushID((const void*)this);
			ImGui::Checkbox("Enabled", &enabled);
			ImGui::Checkbox("Reset transforms before solve", &resetBeforeSolve);
			ImGui::InputInt("Root index", &rootIndex);
...
using namespace cmps_4480_lab_11;
...
namespace cmps_4480_lab_11 {
	...
	void updateNodeMatrix(s_ptr<GLTFNode> node, bool updateChildren = false) {
		...
		if (updateChildren) {
			for (auto& c : node->children) {
				auto& child = meshWithSkin->nodes[c];
				updateNodeMatrix(child, updateChildren);
			}
		}
	}
	...
	void updateNodeMatrices() {
		...
		for (auto& node : meshWithSkin->nodes) {
			...
			/*auto dc = glm::decompose(node->matrix);
			log("Node {0} {1} pos: {2}, {3}, {4}\n", node->index, 
				node->name, dc.position.x, dc.position.y, dc.position.z);*/
		}
	}
}
...
void Lab11::init() {
	...
	// Add default IK chains
	IKChain chain1, chain2;
	...
	ikChains.push_back(chain1);
	ikChains.push_back(chain2);
...
void Lab11::update() {
	...
	// TODO: solve the IK chains
	for (auto& ikChain : ikChains) {
		if (!ikChain.enabled) continue;

		// Get a list of indices between the root and the node.
		std::vector<uint32_t> indices;

		auto node = meshWithSkin->nodes[ikChain.endIndex];

		while (true) {
			indices.push_back(node->index);
			if (node->index == ikChain.rootIndex) break;
			node = node->parent;
		}

		if (ikChain.resetBeforeSolve) {
			for (auto& i : indices) {
				meshWithSkin->nodes[i]->reset();
			}
		}

		// For FABRIK, make sure default distances are known
		for (auto& i : indices) {
			if (ikChain.distances.find(i) == ikChain.distances.end()) {
				auto node_i = meshWithSkin->nodes[i];
				if (node_i->parent) {
					// Get the world position of node i
					vec3 world_i = glm::decompose(node_i->matrix).position;
					vec3 parent_i = glm::decompose(node_i->parent->matrix).position;

					float distance_i = glm::length(world_i - parent_i);
					ikChain.distances[i] = distance_i;
				}
			}
		}
...
void Lab11::render(const mat4& projection, const mat4& view, s_ptr<Framebuffer> framebuffer, bool isShadow) {
	...
	if (meshWithSkin) {
		...
		for (auto& node : meshWithSkin->nodes) {
			if (node->type == +GLTFNodeType::skin || node->type == +GLTFNodeType::mesh) {
				auto& mesh = meshWithSkin->meshes[node->meshIndex];
				...
				for (uint32_t i = 0; i < mesh->primitives.size(); i++) {
					...
					auto usLoc = glGetUniformLocation(meshWithSkin->context->shader->program, "useSkinning");
					if (prim.attributes.contains("WEIGHTS_0")) {
						glUniform1i(usLoc, 1);
					}
					else {
						glUniform1i(usLoc, 0);
						// Get rigid parent index for this mesh primitive. 
						auto rpLoc = glGetUniformLocation(meshWithSkin->context->shader->program, "rigidParent");
						glUniformMatrix4fv(rpLoc, 1, GL_FALSE, glm::value_ptr(node->matrix));
					}
	...
	// Render target
	jr.beginBatchRender(target.shapeType, false, vec4(1.f), isShadow);
	...
	jr.renderBatchWithOwnColor(target, isShadow);
	jr.endBatchRender(isShadow);
	...
	for (auto& skin : meshWithSkin->skins) {
		for (auto& j : skin.joints) {
			auto& node = meshWithSkin->nodes[j];
...
void Lab11::renderUI() {
	...
	if (ImGui::CollapsingHeader("IK chains")) {
		auto ikDelete = ikChains.end();

		int i = 0;
		for (auto ik = ikChains.begin(); ik != ikChains.end(); ik++) {
			std::string chainLabel = fmt::format("Chain {0}", i);
			i++;
			if (ImGui::CollapsingHeader(chainLabel.c_str())) {
				if (ik->renderUI()) {
					ikDelete = ik;
					break;
				}
		...
		if (ikDelete != ikChains.end()) {
			ikChains.erase(ikDelete);
		}
...
ptr_vector<AnimationObject> Lab11::getObjects() {
	ptr_vector<AnimationObject> object_ptrs = { &ground, &target };
	return object_ptrs;
}