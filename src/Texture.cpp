...
Texture::Texture(const uint8_t* bytes, const uint32_t size) {
    //auto& reg = registry();
    //stbi_set_flip_vertically_on_load(1);
    // load and generate the texture
    int width, height, nrChannels;
    //unsigned char* data = stbi_load(filename.c_str(), &width, &height, &nrChannels, 0);
    const uint8_t* data = stbi_load_from_memory(bytes, size, &width, &height, &nrChannels, 0);
    if (data)
    {
        resolution = uvec2(width, height);
        numChannels = nrChannels;
    ...
    {
        log("Failed to load texture from bytes. Not adding texture to registry - check for memleak\n");
    }
...
vec4 Texture::getColorUV(vec2 uv) {
    vec4 result = vec4(0.f);
    if (memory) {
        uv.x = glm::clamp(uv.x, 0.0f, 1.0f);
        uv.y = glm::clamp(uv.y, 0.0f, 1.0f);
        ivec2 pos(glm::round(uv.x * this->resolution.x), glm::round(uv.y * this->resolution.y));
        if (!memory->read((GLvoid*)glm::value_ptr(result), pos.x, pos.y, sizeof(vec4))) {
            log("Unable to read texture {0} at location {1}, {2}\n", id, pos.x, pos.y);
        }
    }
