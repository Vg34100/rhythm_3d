...
#define PROJECT_NAME "4480 codebase"
...
namespace glm
{
	// Helper function to make vecs and quats from a pointer
	template<int L, typename T, qualifier Q>
	inline vec<L, T, Q>  from_pointer(const T * ptr, vec<L, T, Q> & r) {
		if (ptr == nullptr) return r;

		if constexpr (L >= 1) r.x = *ptr;
		if constexpr (L >= 2) r.y = *(ptr + 1);
		if constexpr (L >= 3) r.z = *(ptr + 2);
		if constexpr (L == 4) r.w = *(ptr + 3);
	...
	{
		if (j.is_array()) {
			if constexpr (L >= 1) v.x = j.at(0);
			if constexpr (L >= 2) v.y = j.at(1);
			if constexpr (L >= 3) v.z = j.at(2);
			if constexpr (L >= 4) v.w = j.at(3);
		}
		else if(j.is_object()) {
	...
	template<int L, typename T, qualifier Q>
	inline std::string to_string(const vec<L, T, Q>& v) {
		if constexpr (L == 1) return fmt::format("{0}", v.x);
		if constexpr (L == 2) return fmt::format("{0} {1}", v.x, v.y);
		if constexpr (L == 3) return fmt::format("{0} {1} {2}", v.x, v.y, v.z);
		if constexpr (L == 4) return fmt::format("{0} {1} {2} {3}", v.x, v.y, v.z, v.w);
	}
	...
	inline std::string to_string(const quaternion & q) {
		return fmt::format("{0} {1} {2} {3}", q.x, q.y, q.z, q.w);
	}
...
MAKE_ENUM(AnimationObjectType, int, sphere, box, joint, quad, tri, model);
