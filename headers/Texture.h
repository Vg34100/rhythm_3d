...
MAKE_ENUM(TextureWrapMode, GLenum, Repeat = GL_REPEAT, ClampToEdge = GL_CLAMP_TO_EDGE, ClampToBorder = GL_CLAMP_TO_BORDER, MirroredRepeat = GL_MIRRORED_REPEAT, MirrorClampToEdge = GL_MIRROR_CLAMP_TO_EDGE);
...
MAKE_ENUM(TextureAlphaMode, GLenum, OPAQUE, MASK, BLEND);
...
class Texture {
    public:
		...
		*/
		...
		Texture(const uint8_t * bytes, const uint32_t size);
